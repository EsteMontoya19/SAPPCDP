<?php
    ...
    $sesiones10 = $obj_Sesion->buscarSesionesIDGrupo($idGrupo);
...
$i=0;
foreach ($sesiones10 as $sesion) {
    $arr_asistencia[$i] = $sesion['dia'].'-'.$sesion['mes'];
    $i++;
}
...
function imprimirFecha($contador, $arreglo, $j, $pdf)
{
    ...
    /*  La función imprimirFecha, imprime celdas vacias
        $contador inicializa el contador del for
        $pdf es la instancia que estamos ocupando para imprimir el PDF */
function imprimirColumnasFecha($contador, $pdf, $arreglo, $j)
{
...
for ($i=$numeroUno; $i>0; $i--) {
    // Se cambia el valor de la bandera de acuerdo al número de sesiones que requiere el ciclo
    ...
    foreach ($arr_Inscritos as $inscrito) {
        /*
        // Definition: Número de lista
        $pdf->Cell(8, 5, $k, 1, 0, 'C');

        // Definition: Nombre del profesor
        ...
        */
        // Definition: Columnas de las sesiones
        
        if ($i==1) {
            imprimirColumnasFecha($numeroTres, $pdf, $arr_asistencia);
        }

        // Definition: Columnas de las constancias
        $pdf->Cell(25, 5, '', 1, 0, 'L');
        $pdf->Ln(5);
