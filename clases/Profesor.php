<?php
    class Profesor
    {
		...
		function buscarInstructorxPersona($persona)
		{
			$SQL_Bus_Profesor =
			"
				SELECT usua_id_usuario
				FROM Persona PE, Usuario U
				WHERE PE.pers_id_persona = U.pers_id_persona AND rol_id_rol = 2 AND PE.pers_id_persona = $persona
			";
		...
		function buscarProfesorxPersona($persona)
		{
			...
			"
				...
				FROM Profesor P, Persona PE, Usuario U
				WHERE P.pers_id_persona = PE.pers_id_persona 
					AND PE.pers_id_persona = U.pers_id_persona 
					AND rol_id_rol = 4 
					AND P.pers_id_persona = $persona
			";
		...
		function buscarProfesorxUsuario($usuario)
		{
			...
			"
				SELECT U.usua_id_usuario, P.prof_id_profesor, P.pers_id_persona, P.prof_num_trabajador, 
					...
					PE.pers_apellido_paterno, PE.pers_apellido_materno
				FROM Profesor P, Persona PE, Usuario U
				WHERE P.pers_id_persona = PE.pers_id_persona AND PE.pers_id_persona = U.pers_id_persona AND rol_id_rol = 4 AND U.usua_id_usuario = $usuario
			";
		...
		function buscarNumTrabajador ($numTrabajador, $rol) {
			...
			//? Tratamos de simular polimorfismo para que acepte aún sin el rol
			if (isset($rol)) {
